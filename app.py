...
app.config['RATELIMIT_ENABLED'] = os.environ.get('FLASK_ENV', 'development') == 'production'
...
def get_previous_report(facility_id, commodity_id):
    try:
        ...
        logger.error(f"Error fetching previous report: {str(e)}")
        return None
...
def validate_report_items(items, facility_id):
    ...
    for item in items:
        try:
            ...
            if not commodity:
                errors.append(f"Commodity ID {commodity_id} not found.")
                continue
...
def signup():
    ...
    try:
        ...
        logger.error(f"Error loading signup form choices: {str(e)}")
        flash('Database error loading form options. Please try again.', 'danger')
    ...
    if form.validate_on_submit():
        try:
            ...
            db.session.rollback()
            logger.error(f"Signup failed due to integrity error: {str(e)}")
            flash('Username already exists.', 'danger')
        except SQLAlchemyError as e:
            db.session.rollback()
            logger.error(f"Signup failed: {str(e)}")
            flash('Failed to sign up due to database error. Please try again later.', 'danger')
        except Exception as e:
            db.session.rollback()
...
def forgot_password():
    ...
    if form.validate_on_submit():
        try:
            ...
            flash('Failed to process request due to database error.', 'danger')
...
def admin_users():
    ...
    if request.method == 'POST':
        try:
            ...
            flash('Failed to process action due to database error.', 'danger')
...
def dashboard():
    try:
        ...
        if not user.facility_id:
            logger.error(f"User {user.username} has no assigned facility")
            flash('No facility assigned to your account. Contact an administrator.', 'danger')
            return redirect(url_for('login'))
        ...
        reports = Report.query.filter_by(facility_id=user.facility_id).order_by(Report.report_date.desc()).paginate(page=page, per_page=10, error_out=False)
...
def submit_report():
    ...
    try:
        ...
        logger.error(f"Error loading report form choices: {str(e)}")
        flash('Database error loading form options. Please try again.', 'danger')
        return render_template('submit_report.html', form=form, item_forms={}, commodities=[], user=user)
    ...
    if form.validate_on_submit() and all(f.validate() for f in item_forms.values()):
        try:
            ...
            flash('Failed to submit report due to database error.', 'danger')
    return render_template('submit_report.html', form=form, item_forms=item_forms, commodities=commodities, user=user)
...
def import_reports():
    ...
    if form.validate_on_submit():
        try:
            ...
            flash('Failed to import report due to database error.', 'danger')
        except Exception as e:
            db.session.rollback()
            logger.error(f"Unexpected error during import: {str(e)}", exc_info=True)
            flash('Failed to import report: Invalid file or data.', 'danger')
    return render_template('import_reports.html', form=form, user=user)
...
def view_reports():
    try:
        ...
        reports = query.order_by(Report.report_date.desc()).paginate(page=page, per_page=10, error_out=False)
        ...
        logger.error(f"View reports error: {str(e)}", exc_info=True)
        flash('Database error occurred. Please try again later.', 'danger')
        return redirect(url_for('login'))
    except Exception as e:
        logger.error(f"Unexpected error in view reports: {str(e)}", exc_info=True)
        return render_template('500.html'), 500
